...
import { Search, Download, ShieldCheck, Activity, X, ExternalLink, Copy, Check, Info, Layers, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-5xl max-h-[90vh] bg-background/95 border border-white/10 rounded-2xl glass-panel shadow-neon-primary animate-in zoom-in-95 duration-300 flex flex-col z-[9999] overflow-hidden">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 bg-black/40 backdrop-blur-md border border-white/10 rounded-xl hover:bg-white/10 hover:border-white/20 transition-all text-text-muted hover:text-white z-50 cursor-pointer shadow-lg"
        >
          <X className="w-5 h-5" />
        </button>
...
export default function ResultsTable({ data }) {
  ...
  const handleDownload = () => {
    const exportData = data.map(row => ({
      'Composto': row.Nome_Facil || row.Composto || 'Desconhecido',
      'Composto ID': row.ID || 'N/A',
      'Modo de aquisição': row.Modo_Aquisicao || 'Desconhecido',
      'Score': row.Score || 0,
      'Fragmentação': row['Fragmentation Score'] || 0,
      'Abund. relativa': row.Abundancia_Relativa || 0,
      'Amostra mais abundante': row.Amostra_Mais_Abundante || 'N/A',
      'Descrição': row.Descricao || 'Sem descrição',
      'Classe química': row.ClassyFire_Class && row.ClassyFire_Class !== 'Não documentado' ? row.ClassyFire_Class : row['Categoria química'] || 'Não classificado',
      'Subclasse': row.ClassyFire_Subclass || 'Não documentado',
      'Superclasse': row.ClassyFire_Superclass || 'Não documentado',
      'Fórmula': row.Fórmula || 'N/A',
      'Massa molecular': row.Massa || 0,
      'm/z': row['m/z'] || 0,
      'Tempo de retenção': row['Retention time (min)'] || 0,
      'Taxonomia (Reino)': row.Kingdom || 'Não documentado',
      'Taxonomia (Superfilo)': row.Phylum || 'Não documentado',
      'Doenças associadas (HMDB)': row.HMDB_Disease || 'Não documentado',
      'Tecidos/Biospécimes (HMDB)': row.HMDB_Tissue || 'Não documentado'
    }));

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Resultados IST");
    XLSX.writeFile(wb, "QuimioAnalytics_Analise_IST.xlsx");
  };
